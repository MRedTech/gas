...
function refreshDashboard() {
  ...
  writeTowerGroupVisible_(dashboardSheet, stats.towerDistribution);
...
function createSectionFrames_(sheet) {
  const sections = [
    { title: 'REGISTRATION BY TOWER & GROUP CATEGORY', range: 'I5:L15', titleCell: 'I5' },
    ...
    { title: 'TOWER DISTRIBUTION', range: 'A55:F72', titleCell: 'A55' },
...
function buildStatistics_(filteredRows, allRows) {
  const hourlyTrend = aggregatePeakHour_(filteredRows);
  const monthly = aggregateMonthlyLast4Months_(allRows);
  const category = aggregateCountByLabel_(filteredRows, row => row.category || 'UNKNOWN');
  const groupCategory = aggregateCountByLabel_(filteredRows, row => CATEGORY_GROUPS[row.category] || 'OTHERS');
  const towerDistribution = buildTowerGroupBreakdown_(filteredRows);
  const recordStatus = buildRecordStatus_(filteredRows);
...
function aggregateMonthlyLast4Months_(rows) {
  const now = new Date();
...
function writeTowerGroupVisible_(sheet, towerDistribution) {
  const startRow = 5;
  const startCol = 9; // I
  const totalRows = 10;
  const totalCols = 4;
  ...
  sheet.getRange(startRow, startCol, 1, totalCols)
    .merge()
    .setValue('REGISTRATION BY TOWER & GROUP CATEGORY')
    .setFontWeight('bold')
    .setFontSize(11)
    .setFontColor('#1b5e20')
    .setBackground('#f1f8e9')
  ...
  sheet.getRange(r, startCol, 1, 2)
    ...
    .setFontWeight('bold')
    .setBackground('#f7fbf7');
  sheet.getRange(r, startCol + 2).setValue(totalA).setFontWeight('bold').setHorizontalAlignment('center').setBackground('#f7fbf7');
  sheet.getRange(r, startCol + 3).setValue(totalB).setFontWeight('bold').setHorizontalAlignment('center').setBackground('#f7fbf7');
  ...
  sheet.setColumnWidth(startCol + 1, 125);
...
function writeHelperTables_(sheet, stats) {
  ...
  const tables = [
    ...
    { title: 'MONTHLY', headers: ['MONTH', 'TOTAL'], rows: stats.monthly, colOffset: 3 },
    { title: 'CATEGORY', headers: ['CATEGORY', 'TOTAL'], rows: stats.category, colOffset: 6 },
    { title: 'GROUP CATEGORY', headers: ['GROUP', 'TOTAL'], rows: stats.groupCategory, colOffset: 9 },
    { title: 'TOWER DISTRIBUTION', headers: ['GROUP CATEGORY', 'Tower A', 'Tower B'], rows: stats.towerDistribution.slice(1), colOffset: 12, width: 3 },
    { title: 'RECORD STATUS', headers: ['STATUS', 'TOTAL'], rows: stats.recordStatus, colOffset: 16 }
  ];
...
function buildCharts_(sheet, stats) {
  ...
  const maxMonthly = Math.max(...stats.monthly.map(r => r[1]), 0);
  ...
  const charts = [
    ...
    sheet.newChart()
      ...
      .setOption('title', 'Monthly Registration Trend')
      .setOption('legend', { position: 'none' })
      .setOption('vAxis', {
        textStyle: { fontSize: 10 },
    ...
    sheet.newChart()
      .setChartType(Charts.ChartType.COLUMN)
      .addRange(towerRange)
      .setOption('title', 'Tower Distribution by Category')
