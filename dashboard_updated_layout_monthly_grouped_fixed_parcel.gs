...
function writeHelperTables_(sheet, stats) {
  ...
  const tables = [
    ...
    { title: 'GROUP CATEGORY', headers: ['GROUP', 'TOTAL'], rows: stats.groupCategory, colOffset: 10 },
    { title: 'TOWER DISTRIBUTION', headers: ['GROUP CATEGORY', 'Tower A', 'Tower B'], rows: stats.towerDistribution.slice(1), colOffset: 13, width: 3 },
    { title: 'RECORD STATUS', headers: ['STATUS', 'TOTAL'], rows: stats.recordStatus, colOffset: 17 }
  ];
...
function buildCharts_(sheet, stats) {
  ...
  const groupRange = sheet.getRange(startRow + 1, startCol + 10, Math.max(stats.groupCategory.length + 1, 2), 2);
  const comparisonRange = sheet.getRange(startRow + 1, startCol + 13, stats.towerDistribution.length, 3);
  const recordRange = sheet.getRange(startRow + 1, startCol + 17, Math.max(stats.recordStatus.length + 1, 2), 2);
