...
function createSectionFrames_(sheet) {
  const sections = [
    ...
    { title: 'TOWER COMPARISON BY GROUP CATEGORY', range: 'G17:L33', titleCell: 'G17' },
...
function buildCharts_(sheet, stats) {
  ...
  const comparisonRange = sheet.getRange(startRow + 1, startCol + 12, stats.towerDistribution.length, 3);
