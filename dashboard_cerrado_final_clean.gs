...
function buildDashboard() {
  ...
  if (!sourceSheet) throw new Error("Sheet source 'CERRADO' tidak dijumpai.");
...
function createSectionFrames_(sheet) {
  const sections = [
    ...
    { title: 'CATEGORY', range: 'A35:F53', titleCell: 'A35' },
    { title: 'GROUP CATEGORY', range: 'G35:L53', titleCell: 'G35' },
...
function normalizeCategory_(value) {
  ...
  text = text.replace(/PEDESTRIAN\s*B[-\s]*1/g, 'PEDESTRIAN B1');
