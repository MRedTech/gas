...
function writeHelperTables_(sheet, stats) {
  ...
  const tables = [
    ...
    { title: 'CATEGORY', headers: ['CATEGORY', 'TOTAL', 'LABEL'], rows: stats.category.map(r => [r[0], r[1], String(r[1])]), colOffset: 6, width: 3 },
...
function buildCharts_(sheet, stats) {
  ...
  const categoryRange = sheet.getRange(startRow + 1, startCol + 6, Math.max(stats.category.length + 1, 2), 3);
